...
const LiveViewerGL = ({
  ...
  const applyWindowLevelGamma = useCallback(
    (imageData, width, height, minVal, maxVal, gamma) => {
  ...
  const initWebGL = useCallback(() => {
    ...
    try {
      ...
      if (textureLocation !== null) {
        ...
        console.warn(
          "u_texture uniform not found in shader - creating fallback"
        );
  ...
  const uploadTexture = useCallback((u16Data, width, height) => {
    ...
    if (!gl || !texture) {
      console.warn(
        "WebGL texture upload failed: missing GL context or texture"
      );
      return;
    }
    ...
    if (u16Data.length !== expectedSize) {
      console.warn(
        `WebGL texture upload: data size mismatch. Expected ${expectedSize}, got ${u16Data.length}`
      );
    }
  ...
  const handleMouseMove = useCallback(
    (e) => {
      ...
      console.log(
        `Mouse move delta: ${deltaX}, ${deltaY} => normalized: ${normalizedDeltaX.toFixed(
          3
        )}, ${normalizedDeltaY.toFixed(3)}`
      );
  ...
  const renderCanvas2D = useCallback(
    (u16Data, width, height) => {
      ...
      console.log(
        `Canvas2D window/level: min=${minVal}, max=${maxVal}, gamma=${gamma}`
      );
      ...
      if (elapsed >= 1000) {
        ...
        const compressionRatio =
          counter.bytesReceived > 0
            ? Math.round(
                (counter.compressedBytes / counter.bytesReceived) * 100
              ) / 100
            : 0;
  ...
  const computeHistogram = useCallback(
    (u16Data, width, height) => {
      // Skip if histogram is not enabled
      if (!liveStreamState.showHistogram) return;
      ...
      computeAsync(() => {
        try {
          ...
          console.log("Histogram computed:", {
            bins: binCount,
            totalPixels: u16Data.length,
    ...
    (event) => {
      try {
        ...
        if (packet.dataU16.length > 0) {
          ...
          const step = Math.max(
            1,
            Math.floor(packet.dataU16.length / sampleSize)
          );
  ...
  useEffect(() => {
    ...
    if (canvas && imageSize.width > 0 && imageSize.height > 0) {
      ...
      console.log(
        `ImageSize is invalid (${imageSize.width}x${imageSize.height}) - keeping canvas at current size`
      );
    }
  ...
  const handleWheel = useCallback((event) => {
    ...
    setViewTransform((prev) => {
      ...
      const mouseY = 1 - (y / rect.height) * 2;
      const newTranslateX =
        prev.translateX + (mouseX - prev.translateX) * (1 - zoomFactor);
      const newTranslateY =
        prev.translateY + (mouseY - prev.translateY) * (1 - zoomFactor);
