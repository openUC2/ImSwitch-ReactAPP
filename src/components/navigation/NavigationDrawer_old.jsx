...
import { selectEnabledApps, openAppManager } from "../../state/slices/appManagerSlice.js";
import { APP_REGISTRY } from "../../constants/appRegistry.js";
...
const NavigationDrawer = ({
  ...
  const getEnabledAppsByCategory = (category) => {
    return Object.values(APP_REGISTRY).filter(app => 
      app.category === category && isAppEnabled(app.id)
    );
  };
  
  // Handle App Manager opening
  const handleOpenAppManager = () => {
    dispatch(openAppManager());
    handlePluginChange("AppManager");
  };
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* App Manager - Control enabled applications */}
          {isAppEnabled("appManager") && (
            <DrawerEntry
              icon={<StarIcon />}
              label="App Manager"
              selected={selectedPlugin === "AppManager"}
              onClick={handleOpenAppManager}
              tooltip="App Manager - Control enabled applications"
              color={SIDEBAR_COLORS.essentials}
              collapsed={!sidebarVisible}
              nested={true}
            />
        ...
        {getEnabledAppsByCategory("apps").length > 0 && (
          <DrawerEntry
            icon={<AppsIcon />}
            label="Apps"
          ...
          >
            {/* Render enabled apps dynamically */}
            {getEnabledAppsByCategory("apps").map(app => {
